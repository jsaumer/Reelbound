...
- `scripts/economy/` — **GDScript port of `sim/`**, kept numerically
  ...
  - `play_phase.gd` — the dual-limiter (D6) as a UI-driven state machine
    ...
    winning spin sets `awaiting_gamble_decision` and pauses until the
    caller calls `bank_pending()`/`gamble_pending()`; once quota clears,
    `awaiting_continuation_decision` (D23) pauses every subsequent spin
    until `keep_playing()`/`cash_out()`. Check `has_pending_decision()`
    before calling `spin()` again.
